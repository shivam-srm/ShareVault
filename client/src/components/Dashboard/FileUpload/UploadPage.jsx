...
const FileUploader = () => {
  ...
  const handleFiles = (fileList) => {
    ...
    if (newFiles.length === 0) {
      toast.warn("No valid files (max 10MB each)");
      return;
    }
  ...
  // Accept files from the global drop overlay
  useEffect(() => {
    const onExternal = (e) => {
      if (Array.isArray(e?.detail)) handleFiles(e.detail);
    };
    window.addEventListener("sharevault:external-drop", onExternal);
    return () => window.removeEventListener("sharevault:external-drop", onExternal);
  }, []);
