...
const uploadFiles = async (req, res) => {
  ...
  // Trust the authenticated user, never the client-supplied userId.
...
const uploadFilesGuest = async (req, res) => {
      ...
      try {
           const s3 = new AWS.S3({
            ...
            const processedFiles = await Promise.all(savedFiles.map(async (f) => {
              const command = new GetObjectCommand({
                Bucket: process.env.AWS_BUCKET_NAME,
                Key: `file-share-app/${f.name}`
              });
              let presignedPath = f.path;
              try {
                presignedPath = await getSignedUrl(s3ClientV3, command, { expiresIn: 60 * 60 });
              } catch (err) {}
...
const updateFilePassword = async (req, res) => {
  ...
  try {
    ...
    if (!newPassword || typeof newPassword !== 'string' || newPassword.length < 4) {
      return res.status(400).json({ error: 'New password is required (min 4 chars)' });
    }
...
const searchFiles = async (req, res) => {
  ...
  try {
    ...
    const processedFiles = await Promise.all(files.map(async (file) => {
      ...
      try {
        f.path = await getSignedUrl(s3, command, { expiresIn: 60 * 60 });
      } catch (err) {}
      return f;
    }));
...
const showUserFiles = async (req, res) => {
  // Always scope to the authenticated user; ignore any client-supplied id.
  const userId = req.user?.userId;
...
const file = await File.findOne({ shortUrl });
  ...
  try {
    ...
    try {
      ...
      previewUrl = await getSignedUrl(s3, command, { expiresIn: 60 * 60 });
    } catch (err) {}
...
const getUserFiles = async (req, res) => {
  ...
  try {
    // Only allow reading your own files.
    ...
    const processedFiles = await Promise.all(files.map(async (file) => {
      ...
      try {
        f.path = await getSignedUrl(s3, command, { expiresIn: 60 * 60 }); // 1 hour
      } catch (err) {
        console.error("Error generating presigned URL for file:", f.name, err);
      }
      return f;
    }));
